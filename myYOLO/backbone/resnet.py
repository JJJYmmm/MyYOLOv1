...
class BasicBlock(nn.Module):
    expansion = 1   # expansion是残差结构中输出维度是输入维度的多少倍，BasicBlock没有升维，所以expansion = 1
...
# 这个resnet没有global avgpool and softmax
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        self.inplanes = planes * block.expansion    # 经过一个block后 inplanes需要更新
        for _ in range(1, blocks):
            # block都一样 所以之后不需要更新inplanes
...
# 注意，在调用pytorch 的load_state_dict函数时，我们设置strict参数为False，这样就不会加载进来softmax层的参数了。通过给pretrained参数传True，我们就可以调用PyTorch提供的预训练模型啦。
